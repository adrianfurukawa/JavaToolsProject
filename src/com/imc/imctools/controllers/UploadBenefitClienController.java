...
public class UploadBenefitClienController extends Window {

    private Logger log = LoggerFactory.getLogger(UploadBenefitClienController.class);
    ...
    private List<ICDPOJO> productBft = new ArrayList<ICDPOJO>();
    ...
    private void populate(int offset, int limit) {
         ...
         Session s = Libs.sfDB.openSession();
         try {
             String qry = "select BenefitIMC,BenefitClien from idnhltpf.dbo.BenefitClien "
	        ...
	        if (Messagebox.show("Do you want to clear the list?", "Confirmation", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION)==Messagebox.OK) {
	        	...
	        	Session s = Libs.sfDB.openSession();
	        	 ...
	        	 try{
	             	...
	             	s.beginTransaction();
	        	String qry = " delete idnhltpf.dbo.BenefitClien where  "
           ...
           try {
                ...
                for (int i=0; i< sheet.getLastRowNum()+1; i++) {
                    HSSFRow row = sheet.getRow(i);
				                    ...
				                    String BftImc = colom1.getStringCellValue().trim();
				                    String BftClien = colom2.getStringCellValue().trim();
				                    
				                   
				                    	String qry = "insert into idnhltpf.dbo.BenefitClien "
				                            +"(tahun,polis,BenefitIMC,BenefitClien) " 
				                            +"values "
				                            +" (" 
				                            +"'"+ tPolicyYear.getText() +"',"
    ...
    private boolean saveProcess() {
        ...
        try {
            ...
            for (ICDPOJO icdPOJO : productBft) {
    ...
    public void delete() {
        ...
        if (Messagebox.show("Do you want to remove this exception?", "Confirmation", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, Messagebox.CANCEL)==Messagebox.OK) {
        	...
        	s.beginTransaction();
        	String qry = " delete idnhltpf.dbo.BenefitClien where  "
                    +"tahun ='"+ tPolicyYear.getText() +"' and polis='"+ tPolicyNumber.getText() +"' and BenefitIMC ='"+ Libs.nn(ihm[0]) + "' and BenefitClien='"+ Libs.nn(ihm[1]) +"' ";
        	...
        	productBft.remove(lb.getSelectedItem().getValue());
